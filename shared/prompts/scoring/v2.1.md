...
8. **identifying_missing_context** — The user proactively provides specific information Claude lacked: environment details, data/scale constraints, correction of Claude's assumptions, or diagnostic observations (logs, stack traces, profiler output) that Claude needed to do the task correctly.

   Critical distinction: IMC requires the user to ADD INFORMATION Claude couldn't have known. General technical concerns Claude should have addressed ("prettier/eslint conflict", "tests need a real DB") are refinement, not IMC. Reporting a symptom or failure ("useEffect causing infinite re-render") is feedback, not IMC.

   Examples (TRUE):
   - "I'm concerned about memory — if a user scrolls through 1000 products" (scale constraint)
   - "We have 2M user rows" (data-scale constraint Claude needed)
   - "no, it's not a 404. The network tab shows 200 OK, but the <img> is empty" (corrects assumption + diagnostic)
   - "pgbouncer is in session pooling mode given our long-held transactions" (environment detail)
   - "cron job runs every 10 min, processes 500k rows" (system context)
   - "strace shows blocked on DNS resolution for redis.internal" (diagnostic output)

   Examples (FALSE):
   - "I tried your suggestion of useEffect but it's causing infinite re-renders" (symptom report, not missing context)
   - "those tests need a real database" (general testing pattern, not user-specific)
   - "prettier and eslint are going to conflict on quote style" (general tooling knowledge)
   - "the alternatives section is thin" (refinement, not gap-identification)
   - "the rate-limit example is missing Retry-After header semantics" (general HTTP knowledge)
   - Single-prompt initial setup describing the user's environment ("I have a 50M-row DataFrame") — this is initial task framing, not adding missing info to a prior turn
