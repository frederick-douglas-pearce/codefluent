...
function switchTab(tabName) {
  ...
  const showProject = ['fluency', 'optimizer', 'quickwins', 'usage', 'conversations', 'config'].includes(tabName)
...
document.getElementById('project-filter').addEventListener('change', (e) => {
  ...
  if (state.activeTab === 'conversations') {
    state.conversationsExplorer = null
    loadConversationsExplorer()
  }
  state.configMaturity = null
  state.enforcementGaps = null
...
function renderConversationTokenTable() {
  ...
  function derivedValue(s, key) {
    if (key === 'cache_read_creation_ratio') {
      return s.total_cache_creation_tokens > 0 ? s.total_cache_read_tokens / s.total_cache_creation_tokens : 0
    }
    if (key === 'output_input_ratio') {
...
function renderScoreCorrelation() {
  ...
  // Score color: normalized to data range, red (low) -> amber -> green (high)
  ...
  // --- Chart 1: Cost/Prompt vs Cache Read/Creation Ratio (score as color) ---
  const chart1Data = enriched.filter(s => s.cacheReadCreation != null && s.overall_score != null)
  const chart1All = enriched.filter(s => s.cacheReadCreation != null)

  // --- Chart 2: Cost/Prompt vs Output/Input Ratio (score as color) ---
  const chart2Data = enriched.filter(s => s.outputInput != null && s.overall_score != null)
...
// ISO 8601 week key — matches backend _get_iso_week_key() in main.py
...
async function loadConversationsExplorer() {
  ...
  try {
    ...
    const resp = await fetch(`/api/conversation-analytics?${params}`)
...
async function loadConfigMaturity() {
  ...
  try {
    ...
    const [maturityResp, gapsResp] = await Promise.all([
      fetch(`/api/config-maturity${qs}`),
      fetch(`/api/enforcement-gaps${qs}`),
    ])
    if (!maturityResp.ok) throw new Error(`Config maturity: ${maturityResp.status}`)
    if (!gapsResp.ok) throw new Error(`Enforcement gaps: ${gapsResp.status}`)
    state.configMaturity = await maturityResp.json()
    state.enforcementGaps = await gapsResp.json()
