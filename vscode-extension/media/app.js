...
document.addEventListener('click', (e) => {
  ...
  if (target.classList.contains('copy-hook-btn')) {
    ...
    vscode.postMessage({ type: 'copyToClipboard', text: configText })
    target.textContent = 'Copied!'
...
async function generateHookConfig(idx, btn) {
  ...
  try {
    const result = await postMessageRequest('generateHookConfig', {
      statement: gap.statement,
      hookEvent: gap.suggestedHookEvent || 'PreToolUse',
      matcher: gap.suggestedMatcher || '',
      severity: gap.severity || 'medium',
      lineNumber: gap.lineNumber || gap.line || 0,
      source: gap.source || '',
    })
