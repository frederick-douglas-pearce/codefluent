...
- **Configuration maturity scoring.** The only tool that assesses your `.claude/` project configuration — hooks, rules, commands, skills, MCP, agents, permissions — against best practices. Detects enforcement gaps where your CLAUDE.md states rules but no hooks enforce them, and generates the hook configs for you. Aligned with [Claude Certified Architect (CCA)](https://www.anthropic.com/news/claude-certified-architect) exam competencies.
